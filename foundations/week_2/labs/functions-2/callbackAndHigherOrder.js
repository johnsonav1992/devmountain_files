...
function first(array, printName) {
  return printName(array[0])
}
...
// CODE HERE
function last(array, printName) {
  return printName(array[array.length - 1])
}
...
// CODE HERE           
function contains(array, name, checkForName) {
...
// CODE HERE
function uniq(array, changeArr) {
  let [...uniqArray] = new Set(array)
  return changeArr(uniqArray)
}
...
// CODE HERE 
function each(namesArr, display) {
  namesArr.forEach((name, i) => display(name, i))
}
...
// CODE HERE 
function getUserById(usersArr, idToCheck, printUser) {
  for (let i = 0; i < usersArr.length; i++) {
    if (usersArr[i].id === idToCheck) {
...
function addingFactory(num) {
  let = anotherFunction = (anotherNum) => {
