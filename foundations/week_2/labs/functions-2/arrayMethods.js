...
// CODE HERE
const postTaxPrices = prices.map(price => price * 1.07).map(newPrice => newPrice.toFixed(2))
...
// CODE HERE
const totalPopulation = populations.reduce((currentValue, currentNum) => currentValue + currentNum)
...
// CODE HERE
const myStrongest = monstersInYourPocket.filter(monsterObj => monsterObj.CP > 200)
...
// CODE HERE
const addTax = orders.map(priceObj => priceObj.price * (1 + priceObj.tax)).map(newPrice => newPrice.toFixed(2))
console.log(addTax)
...
// CODE HERE
const bobsTotal = purchases.map(priceObj => priceObj.price).reduce((currentTotal, currentPrice) => currentTotal + currentPrice)
