...
function sayHelloButtonBack() {
    sayHelloButton.style.backgroundColor = '#EFEFEF';
    sayHelloButton.style.color = 'black';
}

sayHelloButton.addEventListener('mouseout', sayHelloButtonBack)
...
const ohMy = () => {
    // YOUR CODE HERE
    axios('http://localhost:3000/animals')
        .then(res => {
...
const repeatMyParam = () => {
    //YOUR CODE HERE
    let param = 'Kelvin is Cool'
    axios(`http://localhost:3000/repeat/${param}`)
        .then(res => {
...
function chooseQuery() {
    ...
    axios('http://localhost:3000/query-test?q=alex&?q=johnson')
        .then(res => {
            console.log(res.data)
        })
}
...
queryButton.addEventListener('click', chooseQuery)
...
// 1. says we sent no queries
// 2. says we sent more than one and returns them
...
function createFood(event) {
    ...
    axios.post('http://localhost:3000/food', body)
        .then(res => {
            ...
            let foodInput = document.querySelector('.food-input')
            let paragraph = document.createElement('p')
            paragraph.textContent = foodInput.value
            let body = document.querySelector('body')
            body.appendChild(paragraph)
        })
